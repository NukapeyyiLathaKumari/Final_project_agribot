...
class VectorStoreModel:
    ...
    def get_or_build(self):
        ...
        if self._db_exists() and current_fp == saved_fp:
            ...
            try:
                ...
                logger.warning(f"Failed to load ChromaDB from disk: {e}. Rebuilding...")
    ...
    def _load_embeddings(self):
        ...
        try:
            ...
            logger.error(
                ...
                "This often means the installed PyTorch binary cannot initialize on Windows. "
                "Install a CPU-only PyTorch wheel or fix the DLL dependency."
            )
            raise RuntimeError(
                "Failed to initialize embeddings. "
                "On Windows, install a CPU-only torch build with: \n"
                "  pip install torch==2.11.0+cpu -f https://download.pytorch.org/whl/cpu \n"
    ...
    def _load_documents(self):
        # web_docs = self._load_web_documents()  # temporarily disabled
        web_docs = []
